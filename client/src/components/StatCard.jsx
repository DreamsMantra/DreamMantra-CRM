export default function StatCard({ icon: Icon, label, value, sub, accent = 'gold', trend, onClick }) {
  const accents = {
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className="dm-stat-card w-full text-left transition hover:border-gold/40 hover:shadow-md">
        {content}
      </button>
    );
  }
  return <div className="dm-stat-card">{content}</div>;
}
