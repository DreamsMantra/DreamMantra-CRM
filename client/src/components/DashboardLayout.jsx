    ...
    <div className="flex min-h-screen dm-dashboard-bg">
      ...
      <div className="flex flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-stone-200 bg-white/90 px-4 py-3 backdrop-blur lg:px-8">
          <button type="button" className="rounded-lg p-2 text-stone-600 lg:hidden" onClick={() => setMobileOpen((o) => !o)}>
            {mobileOpen ? <X /> : <Menu />}
          </button>
          ...
          <div className="flex items-center gap-3">
            {headerActions}
            {badge > 0 && (
              <span className="flex items-center gap-1 rounded-full bg-orange/10 px-3 py-1 text-xs font-semibold text-orange">
                <Bell className="h-3 w-3" /> {badge} new
              </span>
            )}
            <span className="hidden text-sm text-stone-400 lg:block">Dream Mantra Partner CRM</span>
          </div>
        </header>
